...
Real-world patterns that combine the [stream API](/build-on-top/in-process-stream), the [stream server](/build-on-top/stream-server), and the [filesystem reader](/adapters/self-hosted/fs).
...
Docs: https://www.evlog.dev/build-on-top/consumer-recipes
::

## 1. Build a minimal devtool

A live event panel is essentially `EventSource` + a list. Every consumer needs **two** things:

1. The mini stream server's URL (it's on a random port, so discovery first)
2. An SSE connection that decodes `{ evlog, type, data }` envelopes

### URL discovery

Read `.evlog/stream.url` from the project directory:
...
Or hit the `/api/_evlog/stream-info` endpoint from a same-origin browser tab in a Nuxt app:
