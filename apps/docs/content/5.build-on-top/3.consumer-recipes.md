...
A live event panel is essentially `EventSource` + a list. The full wire format and discovery rules — `.evlog/stream.url`, `/api/_evlog/stream-info`, the `{ evlog: '1', type, data }` envelope, and auth — are documented on the [stream page](/build-on-top/stream#wire-format). Each recipe below assumes you've grabbed the URL via either of those mechanisms.
