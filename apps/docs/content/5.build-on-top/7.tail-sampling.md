...
- For multi-field or derived conditions, register an `evlog:emit:keep` hook (Nitro: `nitroApp.hooks.hook('evlog:emit:keep', (ctx) => ...)`); return `true` to keep, `false` to drop, `undefined` to defer to other rules
...
Docs: https://www.evlog.dev/build-on-top/tail-sampling
::
...
- [Plugins](/build-on-top/plugins) when the keep decision is part of a broader feature
