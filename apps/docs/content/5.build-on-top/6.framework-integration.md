...
- Run enrichers via the plugin runner's `runEnrich(ctx)` (or `runEnrichAndDrain(ctx)` if you also want plugin drains to fire) before emitting — never bypass the runner
