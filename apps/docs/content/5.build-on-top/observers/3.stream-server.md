...
evlog ships a tiny HTTP server that exposes the [in-process stream](/build-on-top/observers/in-process-stream) over **Server-Sent Events**. It runs in the same Node process as your app, on its own ephemeral port — your API surface is untouched, and any consumer (browser tab, CLI, Tauri/Electron devtool) can subscribe.
...
2. Subscribes the SSE connections to the [default in-process stream](/build-on-top/observers/in-process-stream#default-singleton) — every wide event flows through.
...
- [Consumer recipes](/build-on-top/observers/consumer-recipes) — copy-paste examples for browser, curl + jq, Node fetch, replay-then-live, aggregation.
- [Stream API](/build-on-top/observers/in-process-stream) — the in-process primitive the server is built on.
