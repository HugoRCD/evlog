...
When several pieces of code in the same process need to share a single stream — typically a framework integration that wires the drain on one side and the [stream server](/build-on-top/observers/stream-server) on the other — use the singleton accessors:
...
The mini [stream server](/build-on-top/observers/stream-server) uses this singleton internally, so anything draining into `getDefaultStream()` automatically reaches all SSE clients.

## Going further

- **Network bridge** — expose this stream over HTTP for browser tabs / CLIs / external devtools. See the [Stream server](/build-on-top/observers/stream-server).
- **Recipes** — concrete consumer patterns (devtool, replay-then-live, aggregation). See [Consumer recipes](/build-on-top/observers/consumer-recipes).
