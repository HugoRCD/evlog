...
- Import `definePlugin` from `evlog` and pick the hooks I need (`onRequestStart`, `enrich`, `drain`, `extendLogger`, `keep`, `onClientLog`)
...
- Import `defineEnricher` from `evlog/toolkit` and call it with `{ name, field?, compute }` — `compute(ctx)` returns the value to merge into `event[field]`, or `undefined` to skip
...
- `compute` is wrapped in try/catch by `defineEnricher`; throwing logs `[evlog/<name>] enrich failed:` and skips the enrichment without blocking the request
