...
- **Don't fan out to a serverless-incompatible target without checking** — the [stream server](/build-on-top/observers/stream-server) reaches every connected client through the in-process stream; it's not a drain

## Going further

- [Drain pipeline](/build-on-top/sinks/drain-pipeline) — the full reference for `createDrainPipeline()` options
- [Custom drains](/build-on-top/sinks/custom-drains) — fanout works with any drain, including ones you write
