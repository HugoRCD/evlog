...
- Single-purpose enricher → [Custom enrichers](/build-on-top/pipeline/custom-enrichers)
- Tail-only logic → [Tail sampling](/build-on-top/pipeline/tail-sampling)
- Drain-only side effect → [Custom drains](/build-on-top/sinks/custom-drains)
- Package your plugin for reuse → [Enrichers as packages](/build-on-top/shared/enrichers-as-packages) (same scaffolding pattern)
