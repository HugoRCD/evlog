...
Not running an HTTP framework? See [Use cases](/use-cases/cli-and-scripts) for CLIs, libraries, jobs, edge, and monorepos.
::
...
| **CLI tool / one-shot script** | `log.*` for steps + `createLogger` for the run summary — see [CLIs and scripts](/use-cases/cli-and-scripts) | Pretty in dev, structured in CI, one summary event for the whole run |
| **Published library** | `createLogger` only — never `initLogger` — see [Libraries](/use-cases/libraries) | Don't pollute the host app's global config or force a drain on consumers |
| **Background job / queue worker / cron** | `createLogger({ jobId, queue })` per invocation — see [Background jobs](/use-cases/background-jobs) | One wide event per job run, perfect for retry analysis |
| **Cloudflare Worker / edge function** | `createWorkersLogger(req)` or `createRequestLogger` — see [Serverless and edge](/use-cases/serverless-and-edge) | Per-request event, no `process` globals required |
...
| **AI agent / LLM call** | `createLogger` + [`createAILogger`](/logging/ai-sdk) | Token usage, tool calls, streaming metrics on the same wide event |
...
| **Monorepo shared package** | Re-export `log` / `createLogger` from a workspace package — see [Monorepos](/use-cases/monorepos) | One `initLogger` call from the leaf app, shared `EnvironmentContext` |
