...
| `prettyErrorFrames` | `boolean` | `true` in dev | Dev-only code snippets around the primary stack frame in pretty error output |
| `prettyErrorStackDepth` | `number` | `2` when compact, `3` otherwise | Max extra stack frames after the snippet (dim gray) |
| `prettyErrorCompact` | `boolean` | `true` in dev | Tighter error block: shorter snippet, compact guidance. Context fields stay on separate tree rows |
| `devErrorHandler` | `'evlog' \| 'nitro'` | `'evlog'` in pretty dev | `'evlog'`: wide event only (no Nitro `[request error]` overlay). `'nitro'`: keep Nitro's Youch output too |
...
### Dev error terminal output

Pretty error blocks (`prettyErrorFrames`, `prettyErrorStackDepth`, `prettyErrorCompact`) run only when `pretty: true` (default in development). Production always emits JSON wide events — no stack snippets or disk reads.

`devErrorHandler` controls whether Nitro's dev Youch overlay runs alongside evlog:

| Value | Terminal output |
|-------|-----------------|
| `'evlog'` | Wide event only (default when `pretty: true` in dev) |
| `'nitro'` | Wide event + Nitro `[request error]` overlay |
