...
### Ship logs with the built-in drain
...
::callout{icon="i-lucide-info" color="info"}
In Nuxt with the evlog module, the server ingest endpoint is auto-registered. For other frameworks, you need to create the endpoint yourself. See the [HTTP drain](/extend/drain-pipeline#server-endpoint) docs for Express and Hono examples.
::
...
See the [HTTP drain](/extend/drain-pipeline) adapter docs for full configuration reference, authentication, and server endpoint examples.
::

## Next Steps

- [HTTP drain](/extend/drain-pipeline) - Batching, retry, and sendBeacon fallback
- [Pipeline](/extend/drain-pipeline) - Advanced pipeline configuration
- [Structured Errors](/learn/structured-errors) - Surface client errors with actionable context
