...
| --- | --- | --- |
| `flags` | citty (auto) + `collect.flags` | Flags you passed — booleans/numbers as values, strings only when allowlisted |
...
| Free-form strings (paths, emails) | Blocked by design | Flags record `"<set>"` instead of the value; hash before `telemetry.set()` |
...
- **Strings** → `"<set>"` (`output: "<set>"`) unless allowlisted in `collect.flags`
