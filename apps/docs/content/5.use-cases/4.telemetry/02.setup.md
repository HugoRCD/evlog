...
| --- | --- | --- |
| `my-tool doctor` | `doctor` | `{}` — nothing was passed, and defaults are not choices |
| `my-tool doctor --json` | `doctor` | `{ json: true }` |
| `my-tool sync --dry-run --output ./out` | `sync` | `{ dryRun: true, output: "<set>" }` — the path itself is never sent |
