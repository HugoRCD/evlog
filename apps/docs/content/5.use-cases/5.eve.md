...
**Resolution:** pass the eve tool `ctx` — `useLogger(ctx)` — when AsyncLocalStorage does not propagate, such as separate hook and tool bundles. With a standard eve agent layout and `agent/hooks/evlog.ts` registered, you should not need it.
