...
Spans and wide events stay joined by attribute, not by trace id: an agent turn has no inbound `traceparent` to inherit, so `evlog.request_id` is what carries across. PostHog's exporter forwards the whole span once it looks AI-related — a name or attribute key starting with `gen_ai.`, `llm.`, `ai.`, or `traceloop.` — which is exactly the model-call spans evlog stamps.
...
Turns then land on the same PostHog person as the LLM traces they produced. See the [PostHog adapter](/integrate/adapters/cloud/posthog#linking-logs-to-people-and-session-replays).
