...
**evlog** is a modern TypeScript logger built for everything you ship. It gives you simple structured logs (a drop-in for `console.log`, pino, or consola), wide events that accumulate context across an operation, and structured errors that explain why they happened — all in one API, all behind the same drain pipeline. Use it in CLIs, libraries, background jobs, edge workers, monorepos, and HTTP handlers without switching loggers.
...
Not running an HTTP framework? evlog ships first-class patterns for [CLIs and scripts](/use-cases/cli-and-scripts), [libraries](/use-cases/libraries), [background jobs](/use-cases/background-jobs), [serverless and edge](/use-cases/serverless-and-edge), and [monorepos](/use-cases/monorepos).
::
...
- [Use cases](/use-cases/cli-and-scripts) - CLIs, libraries, jobs, edge, monorepos
