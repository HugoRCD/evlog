...
evlog provides middleware for NestJS at `evlog/nestjs`. It auto-creates a request-scoped logger accessible via `req.log` or `useLogger()` and emits a wide event when the response finishes.
