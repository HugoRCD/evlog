...
#body
  ::accordion{type="single"}
    ...
    The shape of what you get back, not the speed. pino writes a line per call, so one request leaves four lines you rejoin by request id at query time. evlog accumulates context with `log.set()` as the request runs and emits **one event when it ends**, so the row you query already carries the user, the route, the outcome, and the error. The API mirrors `console`, so the migration itself is mechanical. See [how evlog compares](/reference/vs-other-loggers).
    ::
    ::accordion-item{label="Do I have to change where my logs go?"}
    No, and you do not have to give up the platform you already pay for. evlog produces the events, a drain ships them: Axiom, Datadog, Sentry, Better Stack, PostHog, Loki, ClickHouse, OTLP, or the filesystem. The pipeline batches and retries without blocking your response. If you are building the platform rather than buying one, that same pipeline is where you plug in. See [drain adapters](/integrate/adapters/overview).
    ::
    ::accordion-item{label="How much work is it to adopt?"}
    You migrate one handler at a time. `log.info()` behaves like `console.log`, so nothing breaks on the first commit. Two calls are worth learning: `log.set()`, which adds context to the event in flight, and `createError()`, which attaches a `why` and a `fix` to an error instead of a bare stack. See the [quick start](/start/quick-start).
    ::
    ::accordion-item{label="What does it cost at runtime?"}
    About **3µs per request**, ~6.3 kB gzip, and zero runtime dependencies. In the wide event pattern that is 7.7x faster than pino, because pino serializes four lines where evlog serializes one. On fire-and-forget paths with no context to accumulate the gap closes, and [Performance](/reference/performance) names where evlog loses.
    ::
    ::accordion-item{label="Is it ready for production?"}
    evlog is on v2, MIT licensed, and has **no runtime dependencies**, so there is no tree to audit before you can log. It runs on Node, Bun, Deno, and edge runtimes including Cloudflare Workers. Every pull request runs the benchmark suite through CodSpeed and diffs bundle size against `main`, so a regression is caught before it ships.
    ::
  ::
::
