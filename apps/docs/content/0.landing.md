...
evlog is a modern TypeScript logger that replaces log lines with wide events. One API gives you simple structured logs, structured errors, and lifecycle tracking, across scripts, libraries, jobs, edge, and requests.

### Is evlog a drop-in replacement for pino or console.log?

Yes. Because it keeps a console-like API, evlog is drop-in for `console.log`, `pino`, or `consola`, so you can adopt it without rewriting existing calls.

### How fast is evlog?

evlog adds roughly 3µs of overhead per request, has zero dependencies, and is about 6 kB gzip. It is 7.7x faster than pino in the wide event pattern, sending one correlated event instead of several separate log lines. Benchmarks are tracked on every pull request.

### What is `evlog map`?

`evlog map` is a deterministic observability test for your app. It statically scans every entry point, scores wide-event coverage, and gates in CI, so the same code always produces the same score.

### How is evlog different from an observability platform?

Platforms observe a running app and need traffic, agents, or instrumentation to reveal gaps. evlog is a library with a CLI: it finds missing observability statically, before any traffic exists, when fixes are cheapest.
