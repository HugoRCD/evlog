...
#body
  ::accordion{type="single"}
    ::accordion-item{label="What does evlog do that console.log or pino can't?"}
    It emits one event per request instead of one line per step. You accumulate context with `log.set()` while the request runs, and evlog emits a single correlated event when it ends, whether the handler returned or threw. Four lines you have to stitch back together by request id become one row you can filter on. See [wide events](/learn/wide-events).
    ::
    ::accordion-item{label="How is this different from an observability platform?"}
    A platform reads what your app already sends, so a handler that logs nothing stays invisible until an incident. `evlog map` reads your source and scores every entry point before you deploy. The two stack rather than compete: evlog produces the events, a [drain](/integrate/adapters/overview) ships them to Axiom, Datadog, Sentry, or whatever you already run.
    ::
    ::accordion-item{label="Can I adopt it without rewriting everything?"}
    The API mirrors `console`, so `log.info()` works from the first minute and you migrate one handler at a time. The two calls worth learning are `log.set()` to accumulate context and `createError()` to attach a `why` and a `fix` to an error. See the [quick start](/start/quick-start).
    ::
    ::accordion-item{label="Does it run on edge runtimes?"}
    Yes. evlog has **zero runtime dependencies** and ships ESM only, so there is no transport to assemble and no dependency tree to audit before you can log. It runs on Cloudflare Workers, Vercel Edge, Deno, and Bun. See [Cloudflare Workers](/integrate/frameworks/cloudflare-workers).
    ::
    ::accordion-item{label="What does it cost at runtime?"}
    About **3µs per request** and ~6.3 kB gzip for a typical Node bundle. In the wide event pattern that is 7.7x faster than pino, because pino serializes four lines where evlog serializes one. On fire-and-forget paths with no context to accumulate the gap closes, and [Performance](/reference/performance) says where evlog loses.
    ::
  ::
::
