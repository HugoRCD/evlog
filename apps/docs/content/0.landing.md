...
#body
  ::accordion{type="single"}
    ...
    The volume moves the other way, though what you save depends on how your provider bills. One request leaves one event instead of four or more lines, which is **75% less data** on the wire and one row to index instead of four. Then [sampling](/learn/sampling) drops what you were never going to read and force-keeps errors and slow requests, so what you pay for tracks signal rather than traffic. If your bill is per ingested volume, that is the comparison to run.
    ::
