...
See [Agent Skills](/reference/agent-skills) for the full list. Skill paths in the repo: `skills/build-audit-logs`, `skills/analyze-logs`.
...
Each layer is **opt-in and replaceable**. Every node except `log.audit`, `auditEnricher`, and `auditOnly` / `signed` is shared with regular wide events.
...
::card-group
  ...
  to: /use-cases/audit/recording
  ---
  `log.audit`, `log.audit.deny`, standalone `audit()`, `withAudit`, `defineAuditAction`, `defineAuditCatalog`, and `auditDiff`.
  :::
