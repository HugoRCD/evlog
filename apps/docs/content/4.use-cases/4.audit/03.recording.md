...
Pair this with the action dictionary from [Schema → Action naming](/use-cases/audit/schema#action-naming).
...
**Going further.** The dedicated [Catalogs page](/learn/catalogs) covers the scaling story (single file → folder → feature → npm package) for both error and audit catalogs, plus npm packaging, composition patterns, and the type-augmentation deep dive.
::
