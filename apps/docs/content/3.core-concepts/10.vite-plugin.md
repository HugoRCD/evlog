...
- Injects client-side logging setup in HTML pages
...
For apps that serve HTML (via `index.html`), the plugin automatically injects a `<script>` tag that initializes the client-side logger. This enables `log.info()`, `log.error()`, etc. in browser code.

Configure with:
```typescript [vite.config.ts]
evlog({
  service: 'my-api',
  console: false,     // Suppress browser DevTools output
  transport: {
...
| `console` | `boolean` | `true` | Enable browser console output |
| `transport` | `object` | — | Client log transport config |
