...
Practical patterns for using evlog with Elysia. The `evlog/elysia` plugin auto-creates a request-scoped logger accessible via `log` in route context and `useLogger()`, emitting a wide event when the response completes.
...
Like Hono and Express, Elysia passes `drain` and `enrich` as options to the plugin. Enrich runs first, then drain.
