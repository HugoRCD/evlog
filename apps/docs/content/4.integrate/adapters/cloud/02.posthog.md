...
## What happens between a log and PostHog
...
## Link a log to a person and a session replay
...
## Send custom events alongside logs
...
| `recordShape` | `'json' \| 'compact'` | `'json'` | Event property shape: `compact` flattens nested fields (see [below](#event-format)) |
| `timeout` | `number` | `5000` | Request timeout in milliseconds |
...
### When to use a log and when to use an event
