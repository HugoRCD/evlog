---
title: Fastify
description: The plugin shadows Fastify's built-in pino logger on `request.log` and emits one wide event from the onRequest, onResponse and onError hooks.
navigation:
  title: Fastify
...
::prompt
---
description: Set up evlog in my Fastify app
icon: i-simple-icons-fastify
actions:
  - copy
  - cursor
...
Docs: https://www.evlog.dev/integrate/frameworks/fastify
...
::code-group
```bash [pnpm]
pnpm add evlog fastify
```
```bash [bun]
bun add evlog fastify
```
```bash [yarn]
yarn add evlog fastify
```
```bash [npm]
