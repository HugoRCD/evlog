---
title: Hono
description: One middleware puts a typed logger on `c.get('log')`, defers the event until a streamed body closes, and uses `waitUntil` where the runtime has it.
navigation:
  title: Hono
...
::prompt
---
description: Set up evlog in my Hono app
icon: i-simple-icons-hono
actions:
  - copy
  - cursor
...
Docs: https://www.evlog.dev/integrate/frameworks/hono
...
::code-group
```bash [pnpm]
pnpm add evlog hono @hono/node-server
```
```bash [bun]
...
See the full [HTTP drain](/extend/drain-pipeline) adapter docs for batching, retry, sendBeacon fallback, and authentication options.
::
