---
title: Express
description: One `app.use(evlog())` puts a request logger on `req.log`, emits the wide event when the response finishes, and feeds your error handler.
navigation:
  title: Express
...
Docs: https://www.evlog.dev/integrate/frameworks/express
...
See the full [HTTP drain](/extend/drain-pipeline#http-drain-browser-to-server) adapter docs for batching, retry, sendBeacon fallback, and authentication options.
::
