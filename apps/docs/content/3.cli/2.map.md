---
title: evlog map
description: A static observability score for your app, Lighthouse-style. Scans every entry point in a Nuxt, Nitro, Next.js, or TanStack Start project, scores wide-event coverage, and names the entry points to fix first.
navigation:
  title: map
  icon: i-lucide-radar
links:
...
| `--framework <name>` | detected | Force `nuxt`, `nitro`, `next`, or `tanstack-start` |
...
::prose-accordion{type="single" collapsible}
  ::prose-accordion-item{label="Is `evlog map` deterministic?"}
  Yes. The score comes from static analysis of your source with a versioned rule set, so the same code produces the same score on any machine, on every run. Nothing depends on timing, traffic, or environment.
  ::
  ::prose-accordion-item{label="Can `evlog map` be used in CI?"}
  Yes, and determinism is exactly what makes it safe. `--min-score <n>` blocks a pull request without flakiness, and `--baseline` fails on any regression against the committed `evlog.map.json`, like a snapshot test.
  ::
  ::prose-accordion-item{label="Does `evlog map` need traffic or a running app?"}
  No. It reads your source statically, so it reports before any traffic exists, which is when fixing a blind spot is cheapest. You get a meaningful signal on day one instead of during an incident.
  ::
::
## Next
