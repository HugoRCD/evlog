...
evlog init --yes --extras sampling --sampling high-traffic
...
Ordered by how much they keep. **Errors are kept at 100% in every one of them** — a sampling config that drops errors hides the only events anybody reads at three in the morning.

| Id | Keeps | For |
| --- | --- | --- |
| `all` | Everything | The right answer until volume or cost says otherwise |
| `balanced` | 25% of info, 5% of debug | A usable sample of healthy traffic |
| `high-traffic` | 5% of info, 1% of debug | When info is most of the bill |
| `minimal` | 1% of info, no debug | Very high volume — trends only |
| `errors-only` | No info, no debug | Warnings and errors, nothing else |
...
| `--sampling <id>` | `all`, `balanced` (default), `high-traffic`, `minimal`, `errors-only` |
