...
- For HTTP frameworks with `(ctx, next)`, use `defineFrameworkIntegration` from `evlog/toolkit` — declare `extractRequest(ctx)` returning `{ method, path, headers, requestId? }`, `attachLogger(ctx, logger)`, and an optional storage from `createLoggerStorage()` (`evlog/toolkit/storage`)
...
| `createLoggerStorage(hint)` | Factory returning `{ storage, useLogger }` backed by `AsyncLocalStorage` — import from `evlog/toolkit/storage` (keeps `node:async_hooks` out of the main toolkit barrel) |
