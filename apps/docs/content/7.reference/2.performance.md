...
::prose-accordion{type="single" collapsible}
  ::prose-accordion-item{label="How much overhead does evlog add?"}
  About 3µs per request, which is 0.003ms, orders of magnitude below any HTTP framework or database call.
  ::
  ::prose-accordion-item{label="Is evlog faster than pino?"}
  In the wide event pattern, evlog is about 7.7x faster than pino because it sends one correlated event instead of several separate log lines. On other paths the gap is smaller, and results are tracked on every pull request via CodSpeed, so the numbers stay current.
  ::
  ::prose-accordion-item{label="How do I benchmark evlog myself?"}
  Run `pnpm run bench` in `packages/evlog` for all benchmarks, `pnpm exec vitest bench bench/comparison/` to compare against alternatives, and `pnpm exec tsx bench/scripts/size.ts` for bundle size.
  ::
::
