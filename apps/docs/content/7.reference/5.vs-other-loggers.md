...
## What changes if you already use one of them
...
- **Pick evlog over winston** in any new TypeScript project. winston is older, slower (see [benchmarks](/reference/performance)), and ships none of the modern features (typed events, redaction, structured errors, AI SDK integration).
...
| :feature-label[Runtime level mutation]{tip="Toggle log levels on the fly (e.g. logger.level = 'debug') without a restart — useful for --verbose flags or hot-reload."} | Partial | Yes | Yes | Yes | **pino, consola, winston** |
...
pino lets you pipe a single log to multiple destinations via `pino.multistream`. evlog does the same via [the drain pipeline](/extend/drain-pipeline) (one drain that fans out to N adapters, with batching and retry shared across all of them). The mental model is different. If you've structured your existing code around "stream A is debug to stdout, stream B is warn+ to a file, stream C is error to Sentry", you'll rebuild that as drain-level routing instead.
...
All four become this, the same code whatever the source library:
...
- You have a custom pino transport (e.g. a worker-thread Datadog forwarder you wrote in 2021) you don't want to re-implement as an evlog drain. Most of the [built-in adapters](/integrate/adapters/overview) cover the common destinations, but custom protocols mean a port.
