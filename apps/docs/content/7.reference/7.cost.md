...
## Sampling sits outside the comparison

The calculator does not model sampling, on purpose. Dropping events is not something evlog does and other loggers cannot: you can filter by level in any logger, or sample in the transport. It moves both columns, so folding it in would flatter evlog with a saving that is not evlog's.
