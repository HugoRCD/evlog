...
Next.js evaluates `instrumentation.ts` in both Node.js and Edge runtimes. Use dynamic imports gated behind `NEXT_RUNTIME` to avoid Edge bundling errors from Node.js-only modules (fs drain, adapters, etc.):
