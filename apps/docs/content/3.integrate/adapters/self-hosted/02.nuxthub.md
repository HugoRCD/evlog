...
Every wide event emitted by evlog is stored as a row in the `evlog_events` table. The drain plugin handles both single events and batches (when used with the [pipeline](/extend/drain-pipeline)).
...
- [Adapters](/integrate/adapters/overview) - Send logs to external services alongside NuxtHub storage
- [Pipeline](/extend/drain-pipeline) - Batch events for better database performance
