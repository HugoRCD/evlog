...
The FS adapter requires Node.js (`node:fs`). On the Edge runtime it logs a one-time `[evlog/fs]` warning and skips writes. Use `evlog/memory` or a HTTP adapter for Edge routes.
::
```typescript [Hono]
import { createFsDrain } from 'evlog/fs'
