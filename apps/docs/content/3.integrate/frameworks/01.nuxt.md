...
| `prettyErrorFrames` | `boolean` | `true` in dev | Dev-only code snippets around the primary stack frame |
| `prettyErrorStackDepth` | `number` | `2` when compact, `3` otherwise | Max extra stack frames after the snippet (dim gray) |
| `prettyErrorCompact` | `boolean` | `true` in dev | Tighter error block: shorter snippet. Context fields stay on separate tree rows. |
...
**Which handler?** Use `'evlog'` (default in pretty dev) for a single terminal signal. Use `'nitro'` when you also want Nitro's native Youch overlay alongside the wide event. With `pretty: false`, set `devErrorHandler: 'evlog'` explicitly to suppress Nitro while logging JSON.
::
...
See the [Adapters](/integrate/adapters/overview) and [Enrichers](/use-cases/enrichers) docs for the full list of available drains and enrichers.
::
