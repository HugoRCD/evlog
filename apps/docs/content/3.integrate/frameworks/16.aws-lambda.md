...
AWS Lambda has **no HTTP middleware lifecycle** like Nuxt or Express, so evlog behaves like [standalone TypeScript](/integrate/frameworks/standalone): call `initLogger()` once, create a logger **per invocation** (or per SQS message) with `createLogger()`, then call `log.emit()` when work finishes.
...
Docs: https://www.evlog.dev/integrate/frameworks/aws-lambda
...
- [Standalone TypeScript](/integrate/frameworks/standalone): same `initLogger` + `createLogger` + `emit()` model
- [Configuration](/reference/configuration): `silent`, `env.region` (`AWS_REGION`), drains
- [Wide Events](/learn/wide-events): designing one comprehensive event per unit of work
