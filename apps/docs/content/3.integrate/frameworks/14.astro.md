...
Docs: https://www.evlog.dev/integrate/frameworks/astro
...
On **Cloudflare Workers** (including Astro with `@astrojs/cloudflare`), set `waitUntil` on `createRequestLogger` to your `ExecutionContext#waitUntil` (properly bound), or use [`defineWorkerFetch`](/integrate/frameworks/cloudflare-workers) / [`createWorkersLogger`](/integrate/frameworks/cloudflare-workers) with `{ executionCtx }` on a **Worker `fetch` entry**. Otherwise async drains may never finish after the response is returned.
...
::callout{icon="i-lucide-info" color="info"}
See the [Adapters](/integrate/adapters/overview) docs for all available drain adapters.
::
