...
- Global logger options applied by `register()` (`redact`, `sampling`, `pretty`, …) must be set on `defineNodeInstrumentation()` / `createInstrumentation()`. Once locked, `createEvlog({ redact })` alone is ignored.
::
...
`defineNodeInstrumentation()` and `createInstrumentation()` accept global logger options (`enabled`, `service`, `env`, `pretty`, `silent`, `sampling`, `minLevel`, `stringify`, `drain`, `redact`) plus:

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `captureOutput` | `boolean \| CaptureOutputOptions` | `false` | Capture stdout/stderr as structured log events |
| `redact` | `boolean \| RedactConfig` | `true` in production | Auto-redaction for PII before console output and drains |
