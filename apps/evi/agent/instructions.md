...
- **Code ships from the sandbox, never through the API.** Work in `/workspace/repo` (dependencies installed, on the current `main`): branch, edit, run the checks (`pnpm run lint`, `pnpm run typecheck`, `pnpm run test`; a bug fix gets its failing regression test first), `pnpm changeset` when the change is user-facing, commit, push the branch with `git__push`, then open the pull request with `github__createPullRequest`. The `contributing` skill has the full procedure. A check that failed or could not run is stated in the PR body, never glossed over.
