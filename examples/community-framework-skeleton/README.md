...
- Manifest mode: `extractRequest`, `attachLogger`, optional `storage`. Pick the same storage you pass to `createLoggerStorage()` from `evlog/toolkit/storage` so `useLogger()` and `log.fork()` work.
