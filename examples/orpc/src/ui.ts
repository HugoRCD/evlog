...
const endpoints: Endpoint[] = [
  {
    ...
    code: `fetch('/rpc/health/check', { method: 'POST' }).then(r => r.json())`
  },
  {
    name: 'Get User',
    description: 'User lookup with context accumulation',
    code: `fetch('/rpc/user/getById', {
  method: 'POST',
  ...
  },
  {
    ...
    code: `Promise.all([
  fetch('/rpc/user/getById', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id: '99' })
  }).then(r => r.json()),
  fetch('/rpc/health/check', { method: 'POST' }).then(r => r.json()),
  fetch('/rpc/post/create', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
<body>
  ...
  <script>
    async function runEndpoint(code, name) {
